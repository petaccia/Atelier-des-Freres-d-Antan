...
/**
 * Carte d'avis Google utilisant le même design que TestimonialCard
 * @param {Object} props - Propriétés du composant
 * @param {Object} props.review - Données de l'avis Google
 * @returns {JSX.Element} - Composant de carte d'avis Google
 */
export default function GoogleReviewCard({ review }) {
  ...
  const renderStars = (rating) => {
    return Array(5).fill(0).map((_, i) => (
      <FaStar
        key={i}
        className={`text-xs lg:text-sm ${i < rating ? 'text-amber-500' : 'text-gray-300'}`}
      />
    ));
  };
    ...
    >
      ...
      <div className="flex-shrink-0 z-10">
        <div className="relative w-28 h-28 lg:w-40 lg:h-40 mx-auto lg:mx-0">
          ...
          <Image
            src={review.profile_photo_url ? review.profile_photo_url : "/icons/avatar.png"}
          ...
          <div className="absolute -top-3 -left-3 lg:-top-5 lg:-left-5 bg-white/90 backdrop-blur-sm p-2 lg:p-3 rounded-md lg:rounded-lg shadow-xs lg:shadow-sm">
            <FaQuoteRight className="text-xl lg:text-2xl text-primary" />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col justify-between flex-1 h-5 lg:h-full z-10">
        ...
        <div className="border-t border-stone-200 pt-3 lg:pt-5">
          <h4 className="text-base lg:text-lg font-semibold text-stone-800">{review.author_name}</h4>
